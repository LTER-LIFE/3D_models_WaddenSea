...
ncfile_path <- "C:/Users/qzhan/OneDrive - NIOZ/Attachments/01_LTER-LIFE/03_Model/GETM_ERSEM_SETUPS/Input/"

# --- Load topo (to get grid and Wadden island mask) ---
#nc_topo <- nc_open(paste0(ncfile_path, "/topo/topo_adjusted_dws_200m_2009.nc"))
nc_topo <- nc_open(paste0(ncfile_path, "/topo/topos2_dws_500m.nc"))
...
silt_nc <- rast(paste0(ncfile_path, "/Porosity/sediment_mud_fraction.nc"))
...
ns_nc <- rast(paste0(ncfile_path, "/Porosity/Ben_Sedprop.nc"))
crs(ns_nc)
...
```{r}
ns_por <- rast(paste0(ncfile_path, "/Porosity/Ben_Sedprop.nc"))
...
out_nc <- paste0(ncfile_path, "Ben_Sedprop_500m.nc") # the name needs to be changed to "Ben_Sedprop.nc" which is the name that can be recognized in 3D model
